defmodule Topo.LineLine do
  ...
  defp ring_contains_line?(a, b, offset) do
    ...
    cond do
      !contiguous_subset?(spin, b |> Enum.drop(1) |> Enum.drop(-1)) -> ring_contains_line?(a, b, offset + 1)
      first_and_last_on_line?(spin, b) -> true
      true -> ring_contains_line?(a, b, offset + 1)
    end
  end
  ...
  defp first_and_last_on_line?(a, b) do
    ...
    cond do
      ...
      i1 > in1 -> on_line_after?(a, List.first(b), i1) && on_line_before?(a, List.last(b), in1)
      i1 == in1 -> (on_line_after?(a, List.first(b), i1) && on_line_before?(a, List.last(b), in1)) || (on_line_before?(a, List.first(b), i1) && on_line_after?(a, List.last(b), in1))
    end
  end
  ...
  defp do_linestring_intersects_linestring([a, b | rest], ls) do
    case do_linestring_intersects_segment(ls, {a, b}) do
      :disjoint -> do_linestring_intersects_linestring([b | rest], ls)
      :edge -> :edge
      _ ->
  ...
  defp do_linestring_intersects_segment([a, b | rest], {p1, p2}) do
    case SegSeg.intersection(a, b, p1, p2) do
      {_, :disjoint, _} -> do_linestring_intersects_segment([b | rest], {p1, p2})
      {_, :edge, _} -> :edge
      _ ->
